...
export default function KnowledgeLibrary({ lang, style, pregnancyWeek, riskLevel, alert = null }) {
  ...
  const mergePrioritizedArticles = (baseArticles, priorityArticles = []) => {
    const seen = new Set();
    return [...priorityArticles, ...baseArticles].filter((article) => {
    ...
    if (showEmergency) {
      ...
      if (loadPromise) {
        loadPromise.then(async (baseArticles) => {
          ...
          if (shouldPrioritizeSupport) {
            const recommended = getRiskRecommendedArticles(riskLevel, pregnancyWeek, lang);
            nextArticles = mergePrioritizedArticles(nextArticles, recommended);
          }
          if (shouldPrioritizeEmergency) {
            const emergency = await getEmergencyArticles(lang);
            nextArticles = mergePrioritizedArticles(nextArticles, emergency);
          }
          ...
          {(shouldPrioritizeSupport || shouldPrioritizeEmergency) && (
            <div className="mt-3 rounded-2xl border border-sage/20 bg-[#fffaf2] px-3 py-2 text-xs font-semibold text-clay/72">
              {shouldPrioritizeEmergency
                ? txt(lang, "Emergency and support resources are prioritized first.", "当前优先显示紧急与求助资源。")
                : txt(lang, "Support articles are prioritized based on your current risk trend.", "已根据当前风险趋势优先排列支持内容。")}
            </div>
          )}
