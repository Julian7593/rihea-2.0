...
export function createDoubaoAdapter({ apiKey, baseUrl = "https://ark.cn-beijing.volces.com/api/v3" } = {}) {
  const key = String(apiKey || "").trim();
  const endpointBase = String(baseUrl || "https://ark.cn-beijing.volces.com/api/v3").replace(/\/$/, "");
      ...
      if (!key) {
        return {
          ...
          error: {
            ...
            message: "DOUBAO_API_KEY is missing.",
          },
        };
      }
      ...
      try {
        ...
        if (!response.ok) {
          ...
          return {
            ...
            error: {
              ...
              message: payload?.error?.message || `Doubao HTTP ${response.status}`,
            },
          };
        }
        ...
        return {
          ...
          error: {
            ...
            message: error?.message || "Doubao request failed.",
          },
        };
      } finally {
        timeout.clear();
